...
use crate::remote::BinanceClient;
    ...
    pub async fn start(self, mut rx: broadcast::Receiver<TradeSignal>) {
        ...
        match self.client.get_account().await {
            Ok(info) => {
                ...
                for b in info.balances.iter().filter(|b| b.free.parse::<f64>().unwrap_or(0.0) > 0.0) {
        ...
        loop {
            match rx.recv().await {
                Ok(signal) => {
                    ...
                    match self.client.post_order(&signal.symbol, &signal.side, signal.quantity).await {
                        Ok(order) => {
