...
### Requirement: Supervised Stream Execution
The system SHALL implement the User Data Stream as a supervised `Actor` that sends periodic heartbeats to a central `Supervisor`.

#### Scenario: Actor Failure and Restart
- **WHEN** the `UserStreamActor` fails to send a heartbeat within 3 seconds
- **THEN** the `Supervisor` SHALL terminate the unresponsive actor and spawn a fresh instance.
